...
std::vector< std::vector <cv::Point2f> >  VideoFaceDetector::goodFeatures() const{
    std::vector< std::vector <cv::Point2f> > currentFeatures;
    for (int i=0;i<faces_features.size();i++){
        std::vector <cv::Point2f> feature;
        for (int j=0;j<faces_features[i].size();j++){
            cv::Point2f point;
            point.x = (float)(faces_features[i][j].x / (float)m_scale);
            point.y = (float)(faces_features[i][j].y/ (float)m_scale);
...
void VideoFaceDetector::detectFaceAllSizes( cv::Mat &frame)
{
    ...
    m_faceCascade->detectMultiScale(frame, m_allFaces, 1.1, 3, 0,
                                    cv::Size(frame.rows / 10, frame.cols / 10),
                                    cv::Size(frame.rows , frame.cols));
    ...
    for(auto face_rect : m_allFaces){  
        ...
        this->faces_features.push_back(person.getPoints());
     }

    //  this->start=(m_persons.size()>0);
     
    //  std::vector<cv::Point2f> centers = this->findCenters(this->faces_features);
...
void VideoFaceDetector::detectFacesGoodFeaturesToTrack(cv::Mat frame){
    cv::Mat nextFrame;
    cvtColor(frame, nextFrame, CV_BGR2GRAY);

    std::vector< std::vector<cv::Point2f> > new_faces_features;

    for(auto person : m_persons){  
        person.updateFaceMat(nextFrame);
        new_faces_features.push_back(person.getPoints());
    }
...
cv::Point VideoFaceDetector::getFrameAndDetect(cv::Mat &frame)
{
    *m_videoCapture >> frame;
    ...
    if (!m_foundFace)
        ...
        // if(this->start){
            detectFacesGoodFeaturesToTrack(resizedFrame); // Detect using template matching
        // }
